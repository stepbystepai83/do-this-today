...
with st.expander("1. 先設定你的個人資料", expanded=True):
...
if tasks:
    ...
    st.caption("你可以直接複製這段去發 IG、Threads、限動、貼文。")
else:
    st.info("先按『給我今天該做的事』來生成第一版任務。")

st.divider()

with st.expander("5. 最近行為紀錄"):
    ...
    if not history:
        ...
        for item in reversed(history[-7:]):
            ...
            st.write(f"完成：{', '.join(item['completed_actions']) if item['completed_actions'] else '無'}")
            st.write(f"跳過：{', '.join(item['skipped_actions']) if item['skipped_actions'] else '無'}")
